...
def add_business_days(start_date, days, holidays=[]):
    ...
    while days > 0:
        ...
        if current_date.weekday() < 5 and current_date not in holidays:  # Mon-Fri are considered business days, excluding holidays
            days -= 1
    return current_date
...
# Holiday selection
holidays_input = st.multiselect("Select holidays (if any) within the reporting week:", [])
holidays = [datetime.strptime(date_str, '%Y-%m-%d').date() for date_str in holidays_input]
...
if uploaded_file is not None:
    ...
    for priority in ['P1', 'P2', 'P3', 'P4']:
        ...
        if closed_tickets_count > 0:
            ...
            closed_tickets_df['TAT'] = closed_tickets_df.apply(lambda row: (row['Ticket Closed Time'] - row['Created Time (Ticket)']).days + 1 
                - sum([1 for date in holidays if row['Created Time (Ticket)'].date() <= date <= row['Ticket Closed Time'].date()]), axis=1)
    ...
    for priority in ['P1', 'P2', 'P3', 'P4']:
        ...
        if pending_tickets_count > 0:
            ...
            pending_tickets_df['Target ETA'] = pending_tickets_df['Created Time (Ticket)'].apply(lambda x: get_next_friday(x))
        else:
    ...
    fig_summary = px.bar(summary_report_df, x='Priority', y='Closed Tickets', color='Priority', title="Closed Tickets by Priority (Summary)")
    st.plotly_chart(fig_summary)

    fig_detailed = px.bar(detailed_report_df, x='Ticket Priority', y='Tickets Closed', color='Ticket Priority', title="Closed Tickets by Priority (Detailed)")
    st.plotly_chart(fig_detailed)

    fig_alerts = px.bar(alerts_report_df, x='Priority', y='Closed Tickets', color='Priority', title="Closed Tickets by Priority (Alerts)")
